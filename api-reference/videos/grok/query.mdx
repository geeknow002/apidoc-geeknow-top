...
Grok 系列任务提交后，通过 `GET /v1/videos/{task_id}` 轮询状态。`grok-video-3-pro` / `grok-video-3-max` 与 Grok Imagine 系列共用同一组查询与下载接口。
...
```bash
curl https://td.geeknow.top/v1/videos/video_abc123 \
...
```javascript
const response = await fetch("https://td.geeknow.top/v1/videos/video_abc123", {
  headers: {
...
```bash
curl https://td.geeknow.top/v1/videos/video_abc123/content \
  -H "Authorization: Bearer YOUR_API_KEY" \
  --output result.mp4
```
