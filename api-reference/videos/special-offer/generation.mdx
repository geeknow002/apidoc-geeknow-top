---
title: manxue-2 视频生成
description: 使用 `POST /v1/videos` 调用 `manxue-2` 提交 JSON 视频生成任务。
api: "POST /v1/videos"
---

# manxue-2

`manxue-2` 使用 JSON 请求体提交异步视频任务，支持文生视频、首帧生视频、首尾帧和多参考图生成。
...
func main() {
	...
	req, _ := http.NewRequest("POST", "https://td.geeknow.top/v1/videos", bytes.NewReader(body))
...
公网图片 URL。`manxue-2` 不接收 Base64 data URI。
</ParamField>
