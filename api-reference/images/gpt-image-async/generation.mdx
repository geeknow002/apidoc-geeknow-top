...
`/v1/images/generations/async` 是图像异步生成任务提交入口。请求体采用 OpenAI Images 兼容 JSON 风格；提交成功后保存返回的任务 ID，并通过 `GET /v1/images/generations/async/{taskId}` 查询进度和结果。

- 异步入口为 `POST /v1/images/generations/async`。
- 查询入口为 `GET /v1/images/generations/async/{taskId}`。
- 请求格式为 `application/json`。
- 当前已验证可用模型包括 `gpt-image-2`、`gpt-image-2-pro`、Doubao Seedream 系列和 Gemini 图像模型。
- 字段与统一生图入口基本一致，包括 `model`、`prompt`、`n`、`size`、`image`、`quality`、`response_format`。
...
```bash 文生图 cURL
curl -X POST https://td.geeknow.top/v1/images/generations/async \
...
resp = requests.post(
    "https://td.geeknow.top/v1/images/generations/async",
    headers={
...
const response = await fetch("https://td.geeknow.top/v1/images/generations/async", {
  method: "POST",
  headers: {
    Authorization: "Bearer YOUR_API_KEY",
    "Content-Type": "application/json",
...
模型名称。当前已验证可用值包括 `gpt-image-2`、`gpt-image-2-pro`、`doubao-seedream-4-0-250828`、`doubao-seedream-4-5-251128`、`doubao-seedream-5-0-260128`、`gemini-3-pro-image-preview`、`gemini-2.5-flash-image-preview`、`gemini-3.1-flash-image-preview`。
</ParamField>
