...
<Info>
这些高分档位表示可请求的目标尺寸。若上游不原生接受对应尺寸，网关可能回退到更接近的官方尺寸，并把比例意图附加到 prompt。
</Info>
...
`gpt-image-2-pro` 也支持通过异步入口提交任务：

```http
POST /v1/images/generations/async
```

请求体字段与本页同步生图基本一致，`model` 固定传 `gpt-image-2-pro`。提交成功后读取返回的 `id` / `task_id`，再通过 `GET /v1/images/generations/async/{taskId}` 查询状态和结果。
