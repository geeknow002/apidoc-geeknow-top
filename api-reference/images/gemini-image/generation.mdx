...
```bash
curl -X POST https://td.geeknow.top/v1beta/models/gemini-3-pro-image-preview:generateContent \
...
resp = requests.post(
    "https://td.geeknow.top/v1beta/models/gemini-3-pro-image-preview:generateContent",
    headers={
...
异步入口使用 OpenAI Images 兼容 JSON，而不是本页的 Gemini 原生 `contents[].parts[]` 请求体。`model` 可以传 `gemini-3-pro-image-preview`、`gemini-2.5-flash-image-preview` 或 `gemini-3.1-flash-image-preview`。提交成功后读取返回的 `id` / `task_id`，再通过 `GET /v1/images/generations/async/{taskId}` 查询状态和结果。
